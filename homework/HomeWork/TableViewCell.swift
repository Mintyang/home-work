...
class TableViewCell: UITableViewCell {
    ...
    func setCell(filteredModel:FilteredModel) {
        ...
        if let _ = filteredModel.thumbs?[0].url {
            titleLabelWidth = UIScreen.main.bounds.width - (3 * (20 / 375 * UIScreen.main.bounds.width) + 70 / 375 * UIScreen.main.bounds.width)
        } else {
            titleLabelWidth = UIScreen.main.bounds.width - (2 * (20 / 375 * UIScreen.main.bounds.width))
        }
    ...
    // 設定autoLayout
    func setConstraint(isAdImage: Bool) {
        ...
        if isAdImage {
            ...
            let rightStackViewConstraint = NSLayoutConstraint(item: self.stackView, attribute: .right, relatedBy: .equal, toItem: self, attribute: .right, multiplier: 1, constant: -(20 / 375 * UIScreen.main.bounds.width))
            self.addConstraint(rightStackViewConstraint)
        }
    }
